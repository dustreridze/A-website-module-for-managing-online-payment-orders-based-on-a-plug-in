...
define('MINB_BOOKING_SERVERTYPE', 'TEST'); //TEST
...
function MInB_operate_sendmail($bookingId, $qrLink)
{
    ...
    // Формируем сообщение
    $msg = "Оформлен заказ № {$payment['options']['bookingWord']}" .
        "<br/>Email: {$payment['options']['email']}" .
        "<br/>Описание заказа: {$payment['options']['bookingDescription']}" .
        "<br/>Дата заказа: {$payment['options']['bookingDate']}" .
        "<br/><br/><b>Ссылка для оплаты (действует 7 дней):</b> <a href='{$qrLink}'>{$qrLink}</a>" . // Добавлена ссылка на оплату
        "<br/>Сумма оплаты: " . ($payment['params']['startAmount'] / 100) . " руб" .
        "<br/>Идентификатор операции:  {$payment['params']['session_id']}";
    ...
    $mail->Password = '4iXxNeaPhr2wSSAjgRmq1'; // Пароль от почты (может потребоваться специальный пароль для приложений)
...
function MInB_booking_frontend()
{
	...
	$B = MInB_existsPayment($bookingGetId);

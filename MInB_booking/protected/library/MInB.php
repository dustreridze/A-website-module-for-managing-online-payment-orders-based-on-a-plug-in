...
class MInB_Client
{
	...
	public function getAnswer($payment)
	{
        ...
        file_put_contents('post_log.txt', print_r($logData, true), FILE_APPEND);
		$curl = curl_init();
	...
	public function get()
	{
		...
		switch ( $this->getOperation() )
		{
			case 'CreateOrder':
				...
				$data['date_till'] = date("d.m.Y H:i:s", strtotime('+7 days'));
...
function MInB_createPayment($amount, $options = array())
{
	...
	', array('payment_id' => $id, ':amount' => $parsedAmount));
